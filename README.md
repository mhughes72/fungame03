...
**Web UI features:**
- **Seating chart** — oval table with AI-generated portraits; seats pulse on active speaker and glow green on full consensus
...
All portraits are generated by a single script and stored in `portraits/`. The script uses `gpt-image-1` for most characters and falls back to real Wikipedia photos for figures the model refuses to generate (Hitler, Stalin, Mao, Pol Pot, Lenin, Putin, Xi Jinping, and current tech figures). The `WIKIPEDIA_SOURCES` dict at the top of the script is the place to add more if the model blocks additional characters.

### `generate_portraits.py`

```bash
python generate_portraits.py                           # all characters, cartoon style
...
python generate_portraits.py --style newspaper         # all characters, newspaper style
python generate_portraits.py Newton --style newspaper  # one character, newspaper style
```
...
| Style | Description |
|---|---|
| `cartoon` (default) | Flat vector illustration, bold cartoon, warm colour palette — used by the web UI seating chart |
| `newspaper` | Era-aware: woodcut engraving for pre-1845 figures (Newton, Socrates, Mozart…), vintage sepia press-photo for post-1845 — used by the post-debate newspaper front page |
...
If a generated result doesn't look like the real person, pass `--wiki` to fetch the main Wikipedia photo instead:
...
`--wiki` uses the character's name as the Wikipedia article title. If Wikipedia has no image it falls back to `gpt-image-1` automatically. Characters already in `WIKIPEDIA_SOURCES` always use Wikipedia regardless.

Requires `OPENAI_API_KEY` in `.env` for generation. Wikipedia fetches need no key.
...
After adding a new character to `personas.py`, generate their portrait and thumbnail:

```bash
python generate_portraits.py "Hannah Arendt"
python generate_thumbs.py "Hannah Arendt"
# Newspaper style is generated on demand — only needed if the newspaper front page feature is used:
python generate_portraits.py "Hannah Arendt" --style newspaper --overwrite
```
...
**What to verify:** portrait generator creates images correctly for both styles.

1. Run `python generate_portraits.py Newton` — confirm `portraits/Isaac_Newton.png` is created (cartoon style).
2. Run `python generate_portraits.py Newton --style newspaper --overwrite` — confirm `portraits/Isaac_Newton.png` is regenerated (should use engraving style — Newton died in 1727, well before the 1845 photo-era cutoff).
3. Run `python generate_portraits.py Hitler` — confirm `portraits/Adolf_Hitler.png` is fetched from Wikipedia (no generation call, prints "wikipedia").
4. Run the script again without `--overwrite` — confirm "already exists, skipping" is printed.
5. Run `python generate_portraits.py Xyz` — confirm "No match for 'Xyz'" is printed and no crash occurs.
6. Run `python generate_thumbs.py Newton` — confirm `portrait_thumbs/Isaac_Newton.webp` is created.

**Pass:** images created in `portraits/`, Wikipedia path used for blocked characters, era-aware style fires correctly for newspaper mode, skip and overwrite logic work, thumbnail generation works, bad name handled gracefully.
