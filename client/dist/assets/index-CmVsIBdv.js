...
(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const ze="modulepreload",Ye=function(e){return"/"+e},Se={},re=function(s,t,a){let n=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const l=document.querySelector("meta[property=csp-nonce]"),c=(l==null?void 0:l.nonce)||(l==null?void 0:l.getAttribute("nonce"));n=Promise.allSettled(t.map(u=>{if(u=Ye(u),u in Se)return;Se[u]=!0;const o=u.endsWith(".css"),d=o?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${d}`))return;const w=document.createElement("link");if(w.rel=o?"stylesheet":ze,o||(w.as="script"),w.crossOrigin="",w.href=u,c&&w.setAttribute("nonce",c),document.head.appendChild(w),o)return new Promise((f,k)=>{w.addEventListener("load",f),w.addEventListener("error",()=>k(new Error(`Unable to preload CSS for ${u}`)))})}))}function i(l){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=l,window.dispatchEvent(c),!c.defaultPrevented)throw l}return n.then(l=>{for(const c of l||[])c.status==="rejected"&&i(c.reason);return s().catch(i)})},xe=(e,s,t)=>{const a=e[s];return a?typeof a=="function"?a():Promise.resolve(a):new Promise((n,i)=>{(typeof queueMicrotask=="function"?queueMicrotask:setTimeout)(i.bind(null,new Error("Unknown variable dynamic import: "+s+(s.split("/").length!==t?". Note that variables only represent file names one level deep.":""))))})},G="/api";async function V(e,s){const t=await fetch(`${G}${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!t.ok){let a=`${t.status} ${t.statusText}`;try{const n=await t.json();n.detail&&(a=String(n.detail))}catch{const n=await t.text().catch(()=>"");n&&(a=n)}throw new Error(a)}return t.json()}async function Ke(e){await fetch(`${G}${e}`,{method:"DELETE"})}async function Qe(e=null){const s=e?`${G}/topics?level=${encodeURIComponent(e)}`:`${G}/topics`,t=await fetch(s);if(!t.ok)throw new Error("Failed to load topics");return t.json()}async function Xe(){const e=await fetch(`${G}/characters`);if(!e.ok)throw new Error("Failed to load characters");return e.json()}async function Je(){const e=await fetch(`${G}/styles`);if(!e.ok)throw new Error("Failed to load styles");return e.json()}async function Ze(){const e=await fetch(`${G}/features`);return e.ok?e.json():{}}async function et(e,s,t=!0,a=!0,n=!1,i="university",l="normal",c="normal",u="normal",o="",d=null){return V("/sessions",{characters:e,topic:s,commentator_enabled:t,moderator_enabled:a,diagrams_enabled:n,audience_level:i,philosopher_length:l,commentator_length:c,moderator_length:u,debate_format:o,format_roles:d})}async function tt(e,s,t,a="",n={},i=""){return V(`/sessions/${e}/steer`,{text:s,style:t,evidence:a,drinks:n,producer_directive:i})}async function st(e){return V("/search",{query:e})}async function at(e){return V("/suggest-cast",{topic:e})}async function nt(e){return V("/suggest-topic",{characters:e})}async function rt(e,s){return V(`/sessions/${e}/new-topic`,{topic:s})}async function it(e){return Ke(`/sessions/${e}`)}async function ot(e){return V(`/sessions/${e}/newspaper`,{})}async function lt(e){const s=await fetch(`/api/sessions/${e}/podcast`,{method:"POST"});if(!s.ok){const i=await s.json().catch(()=>({detail:s.statusText}));throw new Error(i.detail||s.statusText)}const t=await s.blob(),a=URL.createObjectURL(t),n=document.createElement("a");n.href=a,n.download="philosophers-bar-podcast.mp3",document.body.appendChild(n),n.click(),n.remove(),URL.revokeObjectURL(a)}async function ct(e,s,t={}){const a={drinks:t};return s!==null&&(a.heat=s),V(`/sessions/${e}/cheat`,a)}function dt(e,s){const t=new EventSource(`${G}/sessions/${e}/stream`);return t.onmessage=a=>{try{const n=JSON.parse(a.data);s(n)}catch{console.error("Unparseable SSE frame:",a.data)}},t.onerror=a=>{console.error("SSE error",a),s({type:"error",data:{text:"Connection lost."}})},()=>t.close()}const le="https://github.com/mhughes72/fungame03";function Oe(e,s){const t=document.createElement("div");t.className="info-overlay",t.innerHTML=`
  ...
  `,document.body.appendChild(t);function a(){t.remove()}t.querySelector(".info-close").addEventListener("click",a),t.addEventListener("click",n=>{n.target===t&&a()}),document.addEventListener("keydown",function n(i){i.key==="Escape"&&(a(),document.removeEventListener("keydown",n))})}function Ae(){Oe("ABOUT",`
    <p class="info-lead">
      ...
      <br><a class="info-link" href="${le}" target="_blank" rel="noopener">${le}</a>
    </p>
  ...
  `)}function He(){Oe("HOW TO PLAY",`
    <p><a class="info-link" href="${le}" target="_blank" rel="noopener">${le}</a></p>
    ...
    <div class="setup-overlay">
      <div class="setup-box setup-box--simple" id="setup-box">
        ...
        <div class="char-list" id="char-list">
          ${(()=>{const r=["Philosophy","Theology","Science","Politics","Arts","Literature","Technology","Media","Psychology"],b={};for(const m of s){const E=m.category||"Other";(b[E]=b[E]||[]).push(m)}return r.filter(m=>b[m]).map(m=>`
              <div class="char-category-group">
                <div class="char-category-label">${H(m)}</div>
                <div class="char-category-cards">
                  ${b[m].map(E=>{const C=E.name.replace(/ /g,"_"),M=E.name.split(" ").map(I=>I[0]).join("").slice(0,2).toUpperCase();return`<label class="char-card"
  ...
  `;const f=e.querySelectorAll("#char-list input[type=checkbox]"),k=e.querySelectorAll(".char-card"),h=e.querySelector("#char-no-results"),L=e.querySelector("#char-filter");e.querySelectorAll(".char-card-img img").forEach(r=>{r.addEventListener("error",function b(){r.removeEventListener("error",b),r.src.includes("/portrait_thumbs/")?(r.addEventListener("error",()=>{r.style.display="none",r.nextElementSibling.style.display="flex"}),r.src=`/portraits/${r.dataset.portrait}.png`):(r.style.display="none",r.nextElementSibling.style.display="flex")})}),L.addEventListener("input",()=>{const r=L.value.toLowerCase().trim();let b=0;k.forEach(m=>{const E=!r||m.dataset.name.includes(r);m.style.display=E?"":"none",E&&b++}),e.querySelectorAll(".char-category-group").forEach(m=>{const E=[...m.querySelectorAll(".char-card")].some(C=>C.style.display!=="none");m.style.display=E?"":"none"}),h.style.display=b===0?"":"none"});const x=document.createElement("div");x.className="char-tooltip",x.style.display="none",document.body.appendChild(x);function S(r){const{desc:b,category:m}=r.currentTarget.dataset;b&&(x.innerHTML=`
      <div class="tt-body">
        ${m?`<span class="tt-category">${H(m)}</span>`:""}
        <span class="tt-desc">${H(b)}</span>
      </div>`,x.style.display="block",_(r))}function _(r){const m=x.offsetWidth,E=x.offsetHeight;let C=r.clientX+14,M=r.clientY+14;C+m>window.innerWidth-14&&(C=r.clientX-m-14),M+E>window.innerHeight-14&&(M=r.clientY-E-14),x.style.left=C+"px",x.style.top=M+"px"}function g(){x.style.display="none"}k.forEach(r=>{r.addEventListener("mouseenter",S),r.addEventListener("mousemove",_),r.addEventListener("mouseleave",g)}),e.querySelectorAll(".format-opt").forEach(r=>{r.addEventListener("mouseenter",S),r.addEventListener("mousemove",_),r.addEventListener("mouseleave",g)});const $=new MutationObserver(()=>{document.body.contains(e)||(x.remove(),$.disconnect())});$.observe(document.body,{childList:!0,subtree:!0}),a&&(e.querySelector("#setup-lengths").style.display="");const q=e.querySelector("#setup-box"),O=e.querySelector("#mode-to-advanced"),A=e.querySelector("#mode-to-simple"),B=e.querySelector(".setup-title");O.addEventListener("click",()=>{q.classList.replace("setup-box--simple","setup-box--advanced"),B.textContent="THE PHILOSOPHER'S EXPERIMENT";const r=e.querySelector('input[name="debate-format"][value="oxford"]');r&&!r.checked&&(r.checked=!0,se())}),A.addEventListener("click",()=>{q.classList.replace("setup-box--advanced","setup-box--simple"),B.textContent=i;const r=e.querySelector('input[name="debate-format"][value=""]');r&&!r.checked&&(r.checked=!0,se())});const N=e.querySelector("#selection-hint"),y=e.querySelector("#start-btn"),v=e.querySelector("#setup-error"),T=e.querySelector("#topic-error"),j=e.querySelector("#start-error");function W(){const r=[...f].filter(E=>E.checked).length,b=z.value.trim().length>0;e.querySelector("#char-list").classList.toggle("char-list--maxed",r>=4),r<2?(N.textContent=r===0?"Select 2 to 4 thinkers":"Select 1 more",N.classList.remove("hint-ok","hint-warn")):r>4?(N.textContent=`Too many — deselect ${r-4}`,N.classList.add("hint-warn"),N.classList.remove("hint-ok")):b?(N.textContent=`${r} selected`,N.classList.add("hint-ok"),N.classList.remove("hint-warn")):(N.textContent="Enter a debate topic",N.classList.remove("hint-ok","hint-warn")),y.disabled=r<2||r>4||!b;const m=e.querySelector("#suggest-topic-btn");m&&(m.disabled=r<2||r>4)}e.querySelector("#char-list"),e.querySelector("#char-filter");const z=e.querySelector("#topic-input");z.addEventListener("input",()=>{T.textContent="",W()}),e.querySelector(".setup-or"),e.querySelector(".topic-label"),e.querySelector(".topic-row"),e.querySelector("#cast-suggestion"),W();const Z=e.querySelector("#suggest-btn"),Y=e.querySelector("#cast-suggestion");let pe=!1;Z.addEventListener("click",async()=>{const r=z.value.trim();if(!r){z.focus();return}T.textContent="",Z.disabled=!0,Z.textContent="thinking…";const b=q.classList.contains("setup-box--advanced");b||(Y.innerHTML='<div class="cs-loading">selecting the best minds for this topic…</div>',Y.style.display="");try{const{picks:m}=await at(r);if(!m||!m.length)return;if(f.forEach(E=>{E.checked=!1}),m.forEach(({name:E})=>{const C=e.querySelector(`#char-list input[value="${CSS.escape(E)}"]`);C&&(C.checked=!0)}),pe=!0,W(),b){e.querySelectorAll('input[name="debate-format"]').forEach(I=>{(I.value==="oxford"||I.value==="cable_news")&&(I.checked=!1)});const C=e.querySelector('input[name="debate-format"][value=""]');C&&(C.checked=!0);const M={id:"_suggested",topic:r,tagline:m.map(I=>I.reason).join(" · "),characters:m.map(I=>I.name),category:"philosophical",audience_level:ue(),format:"",source:"generated"};K=[M],D=0,U.style.display="",te(M),Y.style.display="none",U.scrollIntoView({block:"nearest",behavior:"smooth"})}else{Y.innerHTML='<div class="cs-header">── suggested cast ──</div>'+m.map(C=>`<div class="cs-pick">
              <span class="cs-name">${H(C.name)}</span>
              <span class="cs-reason">${H(C.reason)}</span>
            </div>`).join(""),Y.style.display="";const E=e.querySelector(`.char-card[data-name="${m[0].name.toLowerCase()}"]`);E&&E.scrollIntoView({block:"nearest",behavior:"smooth"})}}catch(m){Y.style.display="none",T.textContent=m.message||"Could not suggest a cast — please try again."}finally{Z.disabled=!1,Z.textContent="Suggest cast ✦"}});const ee=e.querySelector("#suggest-topic-btn"),X=e.querySelector("#topic-suggestion");ee.addEventListener("click",async()=>{const r=[...f].filter(b=>b.checked).map(b=>b.value);if(!(r.length<2)){ee.disabled=!0,ee.textContent="thinking…",X.innerHTML='<div class="cs-loading">finding the perfect flashpoint…</div>',X.style.display="";try{const{topic:b,reason:m}=await nt(r);if(!b)return;z.value=b,X.innerHTML=`<div class="cs-header">── suggested topic ──</div><div class="cs-pick">
          <span class="cs-name">${H(b)}</span>
          <span class="cs-reason">${H(m)}</span>
        </div>`,X.style.display=""}catch(b){console.error("suggest topic failed",b),X.style.display="none"}finally{ee.disabled=!1,ee.textContent="Suggest topic ✦"}}}),f.forEach(r=>r.addEventListener("change",()=>{pe&&(Y.style.display="none",pe=!1),X.style.display="none",W()}));function fe(){const r=e.querySelector('input[name="audience"]:checked'),b=e.querySelector('input[name="phil-length"]:checked'),m=e.querySelector('input[name="comm-length"]:checked'),E=e.querySelector('input[name="mod-length"]:checked'),C=e.querySelector('input[name="debate-format"]:checked');return{commentator:!0,moderator:!0,diagrams:e.querySelector("#toggle-diagrams").checked,audienceLevel:r?r.value:"university",philosopherLength:b?b.value:"normal",commentatorLength:m?m.value:"normal",moderatorLength:E?E.value:"normal",debateFormat:C?C.value:""}}y.addEventListener("click",async()=>{const r=[...f].filter(E=>E.checked).map(E=>E.value),b=e.querySelector("#topic-input").value.trim();T.textContent="",j.textContent="",v.textContent="";const m=y.textContent;y.disabled=!0,y.textContent="opening…";try{await t({characters:r,topic:b,...fe()})}catch{}finally{document.contains(y)&&(y.textContent=m,W())}}),e.querySelector("#topic-input").addEventListener("keydown",r=>{r.key==="Enter"&&!y.disabled&&y.click()}),e.querySelector("#setup-about").addEventListener("click",Ae),e.querySelector("#setup-help").addEventListener("click",He);const U=e.querySelector("#dotd-card"),Fe={heated:"var(--red)",historic:"var(--blue)",philosophical:"var(--gold-dim)",scientific:"var(--blue)",cultural:"var(--amber)",political:"var(--green)"};let be=[],K=[],D=-1;function ue(){const r=e.querySelector('input[name="audience"]:checked');return r?r.value:"university"}function ye(){const r=e.querySelector('input[name="debate-format"]:checked');return r?r.value:""}function we(r){const b=ye();return be.filter(m=>(b==="oxford"?m.format==="oxford":b==="cable_news"?m.format==="cable_news":m.format!=="oxford"&&m.format!=="cable_news")&&(b==="oxford"||b==="cable_news"||m.audience_level===r))}function $e(r,b=null){if(!r.length)return null;const m=b?r.filter(M=>M.id!==b.id):r,E=m.length?m:r,C=[];for(const M of E)C.push(M),M.source==="curated"&&(C.push(M),C.push(M));return C[Math.floor(Math.random()*C.length)]}function te(r){const b=Fe[r.category]||"var(--text-dim)",m=r.format==="oxford",E=r.format==="cable_news";U.classList.toggle("dotd-card--cable",E);const C=q.classList.contains("setup-box--advanced"),I={grade5:"Grade 5",highschool:"High School",university:"University",expert:"Expert"}[r.audience_level]||"",Ue=m?'<span class="dotd-oxford">🎓 Oxford</span>':E?"":C&&I?`<span class="dotd-freeform">${H(I)}</span>`:'<span class="dotd-freeform">Freeform</span>',Ge=m?"":r.source==="curated"?'<span class="dotd-curated">★ curated</span>':'<span class="dotd-generated">AI</span>',Ve=r.roles?`<div class="dotd-roles">
           <span class="dotd-role-prop">For: ${r.roles.proposition.join(", ")}</span>
           <span class="dotd-role-opp">Against: ${r.roles.opposition.join(", ")}</span>
         </div>`:`<div class="dotd-cast">${r.characters.join(" · ")}</div>`,We=E?`
      ...
      <div class="dotd-header">
        <span class="dotd-label">${E?"TONIGHT'S SHOWDOWN":"── CURATED DEBATE ──"}</span>
    ...
    `,U.querySelector("#dotd-start").addEventListener("click",()=>{const ae=m?"oxford":E?"cable_news":"";t({characters:r.characters,topic:r.topic,...fe(),debateFormat:ae,formatRoles:r.roles||null})});const Ee=U.querySelector("#dotd-prev");Ee&&Ee.addEventListener("click",()=>{D>0&&(D--,te(K[D]))});const ke=U.querySelector("#dotd-next");ke&&ke.addEventListener("click",()=>{if(D<K.length-1)D++,te(K[D]);else{const ae=$e(we(ue()),K[D]);ae&&(K.push(ae),D++,te(ae))}})}function se(){const r=$e(we(ue()));r?(K=[r],D=0,te(r)):U.style.display="none"}return Qe().then(r=>{be=r,se()}).catch(()=>{U.style.display="none"}),e.querySelectorAll('input[name="audience"]').forEach(r=>{r.addEventListener("change",()=>{const b=ye();if(b==="oxford"||b==="cable_news"){const m=e.querySelector('input[name="debate-format"][value=""]');m&&(m.checked=!0)}se()})}),e.querySelectorAll('input[name="debate-format"]').forEach(r=>{r.addEventListener("change",se)}),{showError(r){q.classList.contains("setup-box--advanced")?(j.textContent=r,T.textContent=""):(T.textContent=r,j.textContent="")}}}function H(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function P(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function ut(e){const s=(e-.2)/3.8*100,t=Math.max(0,Math.min(100,s)).toFixed(1),a=e>=3?"#4a9b6f":e>=1.5?"#c8a030":"#c83030";return`<div class="ratings-bar-track"><div class="ratings-bar-fill" style="width:${t}%;background:${a}"></div></div>`}const vt={get_them_fighting:"🥊",force_soundbite:"💬",push_narrative:"📢",wrap_it_up:"⏱️",go_soft:"🕊️"},ht={get_them_fighting:"fight",force_soundbite:"soundbite",push_narrative:"narrative",wrap_it_up:"wrap up",go_soft:"go soft"},mt={socratic:"💭",combative:"⚔️","devil's advocate":"😈",koan:"☯️",journalist:"🎤","straw man":"🪆","steel man":"🛡️","last call":"🔔"};function gt(e,s,t="",a,n=null,i=[],l={}){const c=l.moderatorStyleNames??{},u=l.steerTitle??"── STEER THE DEBATE ──",o=l.steerQuitLabel??"Quit game",d=l.steerInputPlaceholder??"Speak into the debate — or leave blank for the moderator…",w=l.steerSubmitLabel??"Steer ▶",f=l.evidencePlaceholder??"Search term — result will be injected as empirical fact…";return new Promise(k=>{const h=document.createElement("div");h.className="steer-drawer",h.innerHTML=`
      ...
      <div class="style-grid" id="style-grid">
        ${s.map(y=>{const v=mt[y.style]??"◆",T=c[y.style]??y.style;return`<button
    ...
    `,(a||document.body).appendChild(h);const x=h.querySelector("#steer-text-input");x.focus();let S=e,_="";h.querySelectorAll(".style-icon-btn").forEach(y=>{y.addEventListener("click",()=>{h.querySelectorAll(".style-icon-btn").forEach(v=>v.classList.remove("style-selected")),y.classList.add("style-selected"),S=y.dataset.style,N()})});const g=h.querySelector("#evidence-toggle"),$=h.querySelector("#evidence-panel"),q=h.querySelector("#evidence-preview"),O=h.querySelector("#evidence-query"),A=h.querySelector("#evidence-search");g.addEventListener("click",()=>{const y=$.style.display==="none";$.style.display=y?"":"none",g.classList.toggle("steer-pill--active",y),y&&O.focus()});async function B(){const y=O.value.trim();if(!(!y||!n)){A.disabled=!0,A.textContent="Searching…",q.style.display="none",_="";try{const v=await n(y);_=v.finding,q.style.display="block",q.innerHTML=`
        ...
        `,q.querySelector("#evidence-accept").addEventListener("click",()=>{q.innerHTML=`<div class="evidence-accepted">✓ Evidence will be injected: ${P(_)}</div>`}),q.querySelector("#evidence-discard").addEventListener("click",()=>{_="",q.style.display="none"})}catch(v){q.style.display="block",q.textContent=`Search failed: ${v.message}`}finally{A.disabled=!1,A.textContent="Search"}}}A.addEventListener("click",B),O.addEventListener("keydown",y=>{y.key==="Enter"&&B()});function N(){h.remove(),k({text:x.value.trim(),style:S,evidence:_})}h.querySelector("#steer-submit").addEventListener("click",N),h.querySelector("#steer-quit").addEventListener("click",()=>{h.remove(),k(null)}),x.addEventListener("keydown",y=>{y.key==="Enter"&&N()})})}function ft(e,s,t={}){const{ratings:a=.8,peak_ratings:n=.8,producer_note:i="",producer_stress:l=0,directives:c=[]}=e,u=["","nervous","stressed","PANICKING","MELTDOWN","FIRE FIRE FIRE"],o=l>0?` (${u[Math.min(l,5)]})`:"",d=l>=3;return new Promise(w=>{const f=document.createElement("div");f.className="steer-drawer",f.innerHTML=`
      <div class="steer-drawer-header">
      ...
      <div class="style-grid" id="directive-list">
        ${c.map(([S,_])=>{const g=vt[S]??"◆",$=ht[S]??S.replace(/_/g," ");return`<button
            class="style-icon-btn"
            data-directive="${P(S)}"
            title="${P($+" — "+_)}"
          >
            <span class="style-icon-glyph">${g}</span>
            <span class="style-icon-name">${P($)}</span>
          </button>`}).join("")}
      </div>
    `,(s||document.body).appendChild(f);const h=f.querySelector("#steer-text-input");h.focus();let L="";f.querySelectorAll("#directive-list .style-icon-btn").forEach(S=>{S.addEventListener("click",()=>{f.querySelectorAll("#directive-list .style-icon-btn").forEach(_=>_.classList.remove("style-selected")),S.classList.add("style-selected"),L=S.dataset.directive,x()})});function x(){const S=h.value.trim();f.remove(),w({text:S,producer_directive:L})}f.querySelector("#steer-submit").addEventListener("click",x),f.querySelector("#steer-quit").addEventListener("click",()=>{f.remove(),w(null)}),h.addEventListener("keydown",S=>{S.key==="Enter"&&x()})})}function ie(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const Le=["arctic","cool","cool","warm","warm","charged","charged","heated","heated","flashpoint","flashpoint"];function bt(e,s,t,a,n=null,i=null,l=null,c=null){return new Promise(u=>{var L,x,S,_;const o={};t.forEach(g=>{o[g]=0});const d=document.createElement("div");d.className="cheat-overlay",d.innerHTML=`
    ...
    `,document.body.appendChild(d);const w=d.querySelector("#cheat-heat-slider"),f=d.querySelector("#cheat-heat-value");w.addEventListener("input",()=>{const g=parseInt(w.value,10);f.textContent=`${g} — ${Le[g]}`}),d.querySelectorAll(".drink-btn").forEach(g=>{g.addEventListener("click",()=>{const $=g.dataset.name,q=g.classList.contains("drink-plus")?1:-1;o[$]=Math.max(0,(o[$]||0)+q);const O=$.replace(/ /g,"_"),A=d.querySelector(`#drink-count-${O}`);A&&(A.textContent=o[$])})});function k(){d.remove(),u()}async function h(){const g=parseInt(w.value,10),$=Object.fromEntries(Object.entries(o).filter(([,O])=>O>0)),q=g!==s;try{await a(e,q?g:null,$)}catch(O){console.error("Cheat failed:",O)}k()}d.querySelector("#cheat-apply").addEventListener("click",h),d.querySelector("#cheat-close").addEventListener("click",k),(L=d.querySelector("#cheat-paper"))==null||L.addEventListener("click",()=>{k(),n()}),(x=d.querySelector("#cheat-podcast"))==null||x.addEventListener("click",()=>{k(),i()}),(S=d.querySelector("#cheat-consensus"))==null||S.addEventListener("click",()=>{k(),c()}),(_=d.querySelector("#cheat-end"))==null||_.addEventListener("click",()=>{k(),l()}),d.addEventListener("click",g=>{g.target===d&&k()})})}function yt(e,s,t={}){e.innerHTML=s.map(u=>{if(t.renderSeat)return t.renderSeat(u,_e(u),ve(u),qe(u),Te(u));const o=_e(u),d=Te(u);return`
      <div class="seat" id="seat-${ve(u)}">
        <div class="seat-portrait-wrap">
          <img class="seat-img" src="${o}" alt="${he(u)}"
               onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" />
          <div class="seat-initials" style="display:none">${he(d)}</div>
        </div>
        <div class="seat-name">${he(qe(u))}</div>
      </div>
    `}).join("");let a=null;function n(u){return e.querySelector(`#seat-${ve(u)}`)}function i(){clearTimeout(a),e.querySelectorAll(".seat").forEach(u=>{u.classList.remove("seat-thinking","seat-speaking")})}function l(u){var o;i(),(o=n(u))==null||o.classList.add("seat-thinking")}function c(u){i();const o=n(u);o&&(o.classList.add("seat-speaking"),a=setTimeout(()=>o.classList.remove("seat-speaking"),3e3))}return{setThinking:l,setSpeaking:c,clearAll:i}}function _e(e){return`/portraits/${e.replace(/ /g,"_")}.png`}function Te(e){return e.split(" ").map(s=>s[0]).join("").slice(0,2).toUpperCase()}function qe(e){return e.split(" ").at(-1)}function ve(e){return e.replace(/[^a-zA-Z0-9]/g,"-").toLowerCase()}function he(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function wt(e,s,t,a,n,i){const l=i.skin??{},c=l.appName??"THE PHILOSOPHER'S BAR";e.innerHTML=`
...
`):T?` — ${T}`:"";console.log(`[${v.channel}] ${v.label}${j}`);break}case"oxford_opening_vote":g={...g,oxford_opening_vote:v},Q(d,{topic:a,...g,debate_phase:S,format_roles:_});break;case"oxford_verdict":Ct(o,v);break;case"phase_update":S=v.debate_phase,_=v.format_roles||{},Q(d,{topic:a,...g,debate_phase:S,format_roles:_});break;case"state":F(o),f=v.moderator_style,k=v.heat??k,v.debate_phase&&(S=v.debate_phase),v.format_roles&&Object.keys(v.format_roles).length&&(_=v.format_roles),g={...v,debate_phase:S,format_roles:_},Q(d,{topic:a,...g});break;case"cable_ratings":$=v.ratings??$,q=v.history??q,Ht(d,$,q);break;case"chyron":v.text&&_t(o,v.text);break;case"breaking_news":v.headline&&Tt(o,v.headline);break;case"producer_whisper":qt(o,v.note,v.stress);break;case"commercial_break":if(x)break;x=!0,$=v.ratings??$,o.scrollTop=o.scrollHeight,ft(v,w,l).then(T=>{x=!1,T===null?Ce(o,{reason:"quit",report:{}},t,N):i.steer(s,T.text,"socratic","",{},T.producer_directive).catch(j=>J(o,`Error: ${j.message}`))});break;case"cable_news_end":if(L)break;L=!0,h&&(h(),h=null),F(o),O.clearAll(),Ce(o,v,t,N);break;case"steer_needed":if(x)break;x=!0,f=v.current_style,v.drift_topic&&kt(o,v.drift_topic,a),o.scrollTop=o.scrollHeight,gt(f,n,Pt(g),w,i.searchEvidence,t,l).then(T=>{x=!1,T===null?oe(o,g,t,N,s,i):(f=T.style,Q(d,{topic:a,...g,moderator_style:T.style}),i.steer(s,T.text,T.style,T.evidence||"",T.drinks||{}).catch(j=>J(o,`Steer error: ${j.message}`)))});break;case"consensus":if(L)break;L=!0,h&&(h(),h=null),F(o),O.clearAll(),Ne(o,v,{onNewTopic(T){i.newTopic(s,T).then(()=>{L=!1,g={turn:0,heat:0,concession_total:0,partial_agreements:[],remaining_disagreements:[],drift_topic:""},e.querySelector(".debate-topic").textContent=T,Q(d,{topic:T,...g,moderator_style:f,points_of_agreement:[]}),O.clearAll(),h=i.openStream(s,B)}).catch(j=>J(o,`Error: ${j.message}`))},onQuit:N},g,s,i,t);break;case"game_over":if(L)break;L=!0,h&&(h(),h=null),F(o),O.clearAll(),oe(o,{...g,...v},t,N,s,i);break;case"bar_beat":A(),Et(o,v.text);break;case"commentator":A(),St(o,v.text);break;case"evidence":A(),xt(o,v.finding);break;case"diagram":A(),Lt(o,v);break;case"system":A(),J(o,v.text);break;case"error":A(),J(o,`⚠ ${v.text}`);break}}function N(){h&&h(),i.deleteSession(s).catch(()=>{}),e.dispatchEvent(new CustomEvent("debate:quit",{bubbles:!0}))}e.querySelector("#about-btn").addEventListener("click",Ae),e.querySelector("#help-btn").addEventListener("click",He),e.querySelector("#sidebar-toggle").addEventListener("click",()=>{const v=e.querySelector(".debate-shell").classList.toggle("sidebar-open");e.querySelector("#sidebar-toggle").textContent=v?"Stats ▲":"Stats"}),e.querySelector("#cheat-btn").addEventListener("click",()=>{L||bt(s,k,t,i.cheat,()=>me(s,i,t),()=>Nt(s,i),()=>{L=!0,h&&(h(),h=null),F(o),O.clearAll(),oe(o,g,t,N,s,i)},()=>{L=!0,h&&(h(),h=null),F(o),O.clearAll(),Ne(o,{summary:"The bar has called it — the evening ends in agreement.",points:g.points_of_agreement||[]},{onNewTopic(y){i.newTopic(s,y).then(()=>{L=!1,g={turn:0,heat:0,concession_total:0,partial_agreements:[],remaining_disagreements:[],drift_topic:""},e.querySelector(".debate-topic").textContent=y,Q(d,{topic:y,...g,moderator_style:f,points_of_agreement:[]}),O.clearAll(),h=i.openStream(s,B)}).catch(v=>J(o,`Error: ${v.message}`))},onQuit:N},g,s,i,t)})}),e.querySelector("#quit-btn").addEventListener("click",()=>{if(L){N();return}g.turn>0?(L=!0,h&&(h(),h=null),oe(o,g,t,N,s,i)):N()}),h=i.openStream(s,B)}function $t(e,{role:s,name:t,content:a,backchannel:n,debate_label:i="",catchphrase:l=""}){const c=document.createElement("div");if(n)c.className="msg msg-bc",c.innerHTML=`<span class="bc-name">${p(t)}:</span> <em>${ne(a)}</em>`;else if(s==="moderator")c.className="msg msg-moderator",c.innerHTML=`<div class="msg-mod-label">― ${p(t)} ―</div><div class="msg-content">${ne(a)}</div>`;else if(s==="user")c.className="msg msg-user",c.innerHTML=`<div class="msg-name msg-name-user">You</div><div class="msg-content">${ne(a)}</div>`;else{const u=`/portraits/${t.replace(/ /g,"_")}.png`,o=t.split(" ").map(f=>f[0]).join("").slice(0,2).toUpperCase(),w=i.includes("Proposition")?"debate-label-prop":"debate-label-opp";c.className="msg msg-philosopher",c.innerHTML=`<div class="msg-avatar"><img class="msg-avatar-img" src="${u}" alt="${p(t)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="msg-avatar-initials" style="display:none">${p(o)}</div></div><div class="msg-body">`+(i?`<div class="msg-debate-label ${w}">${p(i)}</div>`:"")+`<div class="msg-name">${p(t)}</div><div class="msg-content">${ne(a,l)}</div></div>`}R(e,c)}function Et(e,s){const t=document.createElement("div");t.className="msg msg-beat",t.innerHTML=ne(s),R(e,t)}function J(e,s){const t=document.createElement("div");t.className="msg msg-system",s.endsWith("…")?t.innerHTML=p(s.slice(0,-1))+'<span class="typing-dots"><span>.</span><span>.</span><span>.</span></span>':t.textContent=s,R(e,t)}function kt(e,s,t){const a=document.createElement("div");a.className="msg msg-drift",a.innerHTML=`<div class="drift-heading">⇌ Topic Drift</div><div class="drift-new">${p(s)}</div><div class="drift-orig">original: ${p(t)}</div>`,R(e,a)}function St(e,s){const t=document.createElement("div");t.className="msg msg-commentator",t.innerHTML=`<span class="commentator-icon">📢</span><span class="commentator-text">${p(s)}</span>`,R(e,t)}function xt(e,s){const t=document.createElement("div");t.className="msg msg-evidence",t.innerHTML=`<span class="evidence-label">── EVIDENCE ──</span> ${p(s)}`,R(e,t)}function Lt(e,{speaker:s,title:t,thumb_url:a,url:n,page_url:i}){const l=document.createElement("div");l.className="msg msg-diagram",l.innerHTML=`<div class="diagram-label">${p(s)} produces a diagram</div><a class="diagram-link" href="${p(i)}" target="_blank" rel="noopener"><img class="diagram-img" src="${p(a)}" alt="${p(t)}" /><div class="diagram-caption">${p(t)}</div></a>`,R(e,l)}function _t(e,s){const t=document.createElement("div");t.className="msg msg-chyron",t.textContent=s,R(e,t)}function Tt(e,s){const t=document.createElement("div");t.className="msg msg-breaking-news",t.innerHTML=`<div class="breaking-label">🔴 BREAKING NEWS</div><div class="breaking-headline">${p(s)}</div>`,R(e,t)}function qt(e,s,t){if(!s)return;const a=document.createElement("div");a.className="msg msg-producer-whisper";const n=t>=3?"[PRODUCER!!!]":"[PRODUCER]";a.innerHTML=`<span class="producer-tag${t>=3?" producer-tag-high":""}">${n}</span> ${p(s)}`,R(e,a)}function Ce(e,{reason:s,report:t={}},a,n,i,l){F(e);const c=s==="viral",o=c?"📺 SHOW WENT VIRAL":s==="cancelled"?"📺 SHOW CANCELLED":"📺 LAST CALL",d=c?"cable-end-viral":"cable-end-cancelled",{final_ratings:w=0,peak_ratings:f=0,turn_count:k=0,breaking_news_count:h=0,network_offers:L={},catchphrases:x={},guest_stats:S={}}=t,_=Object.entries(L).length?`<div class="end-section cable-end-offers">
        ...
        ${Object.entries(x).map(([O,A])=>{const N=(S[O]||{}).catchphrase_count||0;return`<div class="cable-catchphrase-row">
            <span class="cable-cp-name">${p(O)}:</span>
            <span class="cable-cp-phrase">"${p(A)}"</span>
            <span class="cable-cp-count">${N}×</span>
          </div>`}).join("")}
      </div>`:"",$=k?`<div class="end-scoreboard">
        <div class="end-stat">
          <span class="end-stat-num">${k}</span>
  ...
  `,R(e,q),q.querySelector("#cable-end-leave").addEventListener("click",n)}function Ct(e,{winner:s,proposition_open:t,proposition_final:a,margin:n,persona_verdicts:i,verdict:l}){const c=s==="proposition"?"PROPOSITION WINS":"OPPOSITION WINS",u=s==="proposition"?"oxford-verdict-prop":"oxford-verdict-opp",o=a===50,d=Math.min(t,a),f=Math.max(t,a)-d,k=a>t,h=k?"var(--green)":"var(--amber)",L=(n>=0?"+":"")+n+" pts",x=`
    <div class="oxford-shift-wrap">
      ...
      <div class="oxford-shift-track">
        <div class="oxford-shift-fill" style="left:${d}%;width:${f}%;background:${h}"></div>
      ...
      <div class="oxford-shift-margin" style="color:${h}">${k?"→":"←"} ${L}</div>
    </div>
  `,S=o?'<div class="oxford-verdict-tie">The motion falls — a tie goes to the opposition.</div>':"",_=document.createElement("div");_.className="oxford-verdict-card",_.innerHTML=`
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      ${n.map(c=>typeof c=="object"&&c!==null?`<div class="end-tension">
        ...
        <div class="oxford-opening-detail">
          ...
          <ul class="oxford-opening-leanings">
            ${(f.persona_leanings||[]).map($=>`<li>${p($)}</li>`).join("")}
          </ul>
        </div>
      </details>`:"";let S=`
    ${h}
  ...
  `}function je(e){return e<=2?"#4a7ab5":e<=4?"#8a9040":e<=6?"#c8a030":e<=8?"#c86030":"#c83030"}function De(e){return e<=2?"cool":e<=4?"warm":e<=6?"charged":e<=8?"heated":"flashpoint"}function Mt(e){return e<=2?"#4a7ab5":e<=5?"#4a9b6f":e<=8?"#6abf8a":"#a8e6bf"}function Rt(e){return e===0?"none":e<=2?"rare":e<=5?"some":e<=8?"frequent":"flowing"}function Pt(e,s){const{turn:t,heat:a,partial_agreements:n,remaining_disagreements:i,drift_topic:l}=e;if(!t)return"The debate is just getting started.";if(l)return`The conversation has drifted from the original topic toward ${l}.`;const c=n||[],u=i||[];if(c.length&&u.length){const d=c[0],w=u[0],f=d.participants.join(" and "),k=typeof w=="object"?w.topic:String(w);return`${f} are finding common ground, but the group remains divided on ${k}.`}if(c.length){const d=c[0];return`${d.participants.join(" and ")} are converging on ${d.on}, ${a>=6?"though tempers are running high":"with the room following closely"}.`}if(u.length){const d=u[0];return typeof d=="object"?`${d.participant_a} and ${d.participant_b} are sharply divided over ${d.topic}.`:`The room is deadlocked — ${String(d)}.`}const o=a>=8?"at flashpoint":a>=5?"heating up":a>=3?"warming up":"still feeling each other out";return`${t} turns in, no clear alignments yet — the room is ${o}.`}const ce=document.querySelector("#app");let de={},ge={};const It=new Set(["production","development","staging"]);async function jt(){const e=It.has("production")?"default":"production",[s]=await Promise.all([xe(Object.assign({"./skins/default/skin.js":()=>re(()=>import("./skin-D7HGMV-G.js"),[]),"./skins/kids/skin.js":()=>re(()=>import("./skin-DZ74tMJe.js"),[])}),`./skins/${e}/skin.js`,4),xe(Object.assign({"./skins/default/theme.css":()=>re(()=>Promise.resolve({}),__vite__mapDeps([0])),"./skins/kids/theme.css":()=>re(()=>Promise.resolve({}),__vite__mapDeps([1]))}),`./skins/${e}/theme.css`,4)]);return s}async function Be(){let e,s;try{[e,s,de]=await Promise.all([Xe(),Je(),Ze()])}catch(n){ce.innerHTML=`<div class="fatal-error">Could not reach the server.<br/>${n.message}</div>`;return}const t=!!de.local,a=pt(ce,e,async({characters:n,topic:i,commentator:l=!0,moderator:c=!0,diagrams:u=!1,audienceLevel:o="university",philosopherLength:d="normal",commentatorLength:w="normal",moderatorLength:f="normal",debateFormat:k="",formatRoles:h=null})=>{try{const L=await et(n,i,l,c,u,o,d,w,f,k,h);Dt(L.session_id,n,i,s)}catch(L){throw a.showError(L.message),L}},{isLocal:t,skin:ge})}function Dt(e,s,t,a){wt(ce,e,s,t,a,{skin:ge,steer:tt,cheat:ct,deleteSession:it,newTopic:rt,openStream:dt,searchEvidence:st,fetchNewspaper:ot,exportPodcast:de.podcast?lt:null,isLocal:!!de.local}),ce.addEventListener("debate:quit",()=>Be(),{once:!0})}jt().then(e=>{ge=e}).catch(()=>{}).finally(()=>Be());
