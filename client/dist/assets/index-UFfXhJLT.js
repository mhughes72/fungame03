(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))n(r);new MutationObserver(r=>{for(const i of r)if(i.type==="childList")for(const p of i.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&n(p)}).observe(document,{childList:!0,subtree:!0});function t(r){const i={};return r.integrity&&(i.integrity=r.integrity),r.referrerPolicy&&(i.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?i.credentials="include":r.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(r){if(r.ep)return;r.ep=!0;const i=t(r);fetch(r.href,i)}})();const N="/api";async function O(e,s){const t=await fetch(`${N}${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!t.ok){const n=await t.text();throw new Error(`${t.status} ${t.statusText}: ${n}`)}return t.json()}async function re(e){await fetch(`${N}${e}`,{method:"DELETE"})}async function ie(e=null){const s=e?`${N}/topics?level=${encodeURIComponent(e)}`:`${N}/topics`,t=await fetch(s);if(!t.ok)throw new Error("Failed to load topics");return t.json()}async function oe(){const e=await fetch(`${N}/characters`);if(!e.ok)throw new Error("Failed to load characters");return e.json()}async function ce(){const e=await fetch(`${N}/styles`);if(!e.ok)throw new Error("Failed to load styles");return e.json()}async function le(){const e=await fetch(`${N}/features`);return e.ok?e.json():{}}async function de(e,s,t=!0,n=!0,r=!1,i="university"){return O("/sessions",{characters:e,topic:s,commentator_enabled:t,moderator_enabled:n,diagrams_enabled:r,audience_level:i})}async function pe(e,s,t,n="",r={}){return O(`/sessions/${e}/steer`,{text:s,style:t,evidence:n,drinks:r})}async function ue(e){return O("/search",{query:e})}async function me(e,s){return O(`/sessions/${e}/new-topic`,{topic:s})}async function ve(e){return re(`/sessions/${e}`)}async function he(e){return O(`/sessions/${e}/newspaper`,{})}async function ge(e){const s=await fetch(`/api/sessions/${e}/podcast`,{method:"POST"});if(!s.ok){const i=await s.json().catch(()=>({detail:s.statusText}));throw new Error(i.detail||s.statusText)}const t=await s.blob(),n=URL.createObjectURL(t),r=document.createElement("a");r.href=n,r.download="philosophers-bar-podcast.mp3",document.body.appendChild(r),r.click(),r.remove(),URL.revokeObjectURL(n)}async function fe(e,s,t={}){const n={drinks:t};return s!==null&&(n.heat=s),O(`/sessions/${e}/cheat`,n)}function be(e,s){const t=new EventSource(`${N}/sessions/${e}/stream`);return t.onmessage=n=>{try{const r=JSON.parse(n.data);s(r)}catch{console.error("Unparseable SSE frame:",n.data)}},t.onerror=n=>{console.error("SSE error",n),s({type:"error",data:{text:"Connection lost."}})},()=>t.close()}const Y="https://github.com/mhughes72/fungame03";function J(e,s){const t=document.createElement("div");t.className="info-overlay",t.innerHTML=`
  ...
  `;const n=e.querySelectorAll("#char-list input[type=checkbox]"),r=e.querySelectorAll(".char-row"),i=e.querySelector("#char-no-results"),p=e.querySelector("#char-filter");p.addEventListener("input",()=>{const o=p.value.toLowerCase().trim();let E=0;r.forEach(S=>{const T=!o||S.dataset.name.includes(o);S.style.display=T?"":"none",T&&E++}),i.style.display=E===0?"":"none"});const a=document.createElement("div");a.className="char-tooltip",a.style.display="none",document.body.appendChild(a);function l(o){const{desc:E,category:S,portrait:T}=o.currentTarget.dataset;if(!E)return;const q=`/portraits/${T}.png`;a.innerHTML=`
      ...
      </div>`,a.style.display="block",m(o)}function m(o){const S=a.offsetWidth,T=a.offsetHeight;let q=o.clientX+14,_=o.clientY+14;q+S>window.innerWidth-14&&(q=o.clientX-S-14),_+T>window.innerHeight-14&&(_=o.clientY-T-14),a.style.left=q+"px",a.style.top=_+"px"}function u(){a.style.display="none"}r.forEach(o=>{o.addEventListener("mouseenter",l),o.addEventListener("mousemove",m),o.addEventListener("mouseleave",u)});const c=new MutationObserver(()=>{document.body.contains(e)||(a.remove(),c.disconnect())});c.observe(document.body,{childList:!0,subtree:!0});const v=e.querySelector("#selection-hint"),b=e.querySelector("#start-btn"),k=e.querySelector("#setup-error");function g(){const o=[...n].filter(E=>E.checked).length;o<2?(v.textContent=o===0?"Select 2 to 4 thinkers":"Select 1 more",v.classList.remove("hint-ok","hint-warn")):o>4?(v.textContent=`Too many — deselect ${o-4}`,v.classList.add("hint-warn"),v.classList.remove("hint-ok")):(v.textContent=`${o} selected`,v.classList.add("hint-ok"),v.classList.remove("hint-warn")),b.disabled=o<2||o>4}g(),n.forEach(o=>o.addEventListener("change",g));function y(){const o=e.querySelector('input[name="audience"]:checked');return{commentator:e.querySelector("#toggle-commentator").checked,moderator:e.querySelector("#toggle-moderator").checked,diagrams:e.querySelector("#toggle-diagrams").checked,audienceLevel:o?o.value:"university"}}b.addEventListener("click",()=>{const o=[...n].filter(S=>S.checked).map(S=>S.value),E=e.querySelector("#topic-input").value.trim()||"What is the nature of justice?";k.textContent="",t({characters:o,topic:E,...y()})}),e.querySelector("#topic-input").addEventListener("keydown",o=>{o.key==="Enter"&&!b.disabled&&b.click()}),e.querySelector("#setup-about").addEventListener("click",V),e.querySelector("#setup-help").addEventListener("click",X);const h=e.querySelector("#dotd-card"),$={heated:"var(--red)",historic:"var(--blue)",philosophical:"var(--gold-dim)",scientific:"var(--blue)",cultural:"var(--amber)",political:"var(--green)"};let x=[],L=null;function f(){const o=e.querySelector('input[name="audience"]:checked');return o?o.value:"university"}function w(o){return x.filter(E=>E.audience_level===o)}function H(o,E=null){if(!o.length)return null;const S=E?o.filter(_=>_.id!==E.id):o,T=S.length?S:o,q=[];for(const _ of T)q.push(_),_.source==="curated"&&(q.push(_),q.push(_));return q[Math.floor(Math.random()*q.length)]}function R(o){L=o;const E=$[o.category]||"var(--text-dim)",S=o.source==="curated"?'<span class="dotd-curated">★ curated</span>':'<span class="dotd-generated">AI</span>';h.innerHTML=`
    ...
    `,h.querySelector("#dotd-start").addEventListener("click",()=>{t({characters:o.characters,topic:o.topic,...y()})}),h.querySelector("#dotd-new").addEventListener("click",()=>{const T=H(w(f()),L);T&&R(T)})}function U(){const o=H(w(f()));o?R(o):h.style.display="none"}return ie().then(o=>{x=o,U()}).catch(()=>{h.style.display="none"}),e.querySelectorAll('input[name="audience"]').forEach(o=>{o.addEventListener("change",U)}),{showError(o){k.textContent=o}}}function A(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function M(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function we(e,s,t="",n,r=null,i=[]){return new Promise(p=>{const a=document.createElement("div");a.className="steer-drawer",a.innerHTML=`
    ...
    `,(n||document.body).appendChild(a);const m=a.querySelector("#steer-text-input"),u=a.querySelector("#evidence-query"),c=a.querySelector("#evidence-search"),v=a.querySelector("#evidence-preview");m.focus();let b=e,k="";async function g(){const h=u.value.trim();if(!(!h||!r)){c.disabled=!0,c.textContent="Searching…",v.style.display="none",k="";try{const $=await r(h);k=$.finding,v.style.display="block",v.innerHTML=`
        ...
        `,v.querySelector("#evidence-accept").addEventListener("click",()=>{v.innerHTML=`<div class="evidence-accepted">✓ Evidence will be injected: ${M(k)}</div>`}),v.querySelector("#evidence-discard").addEventListener("click",()=>{k="",v.style.display="none"})}catch($){v.style.display="block",v.textContent=`Search failed: ${$.message}`}finally{c.disabled=!1,c.textContent="Search"}}}c.addEventListener("click",g),u.addEventListener("keydown",h=>{h.key==="Enter"&&g()}),a.querySelectorAll(".style-item").forEach(h=>{h.addEventListener("click",()=>{a.querySelectorAll(".style-item").forEach($=>$.classList.remove("style-selected")),h.classList.add("style-selected"),b=h.dataset.style,y()})});function y(){const h=m.value.trim();a.remove(),p({text:h,style:b,evidence:k})}a.querySelector("#steer-submit").addEventListener("click",y),a.querySelector("#steer-quit").addEventListener("click",()=>{a.remove(),p(null)}),m.addEventListener("keydown",h=>{h.key==="Enter"&&y()})})}function I(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const Q=["arctic","cool","cool","warm","warm","charged","charged","heated","heated","flashpoint","flashpoint"];function $e(e,s,t,n,r=null,i=null){return new Promise(p=>{var b,k;const a={};t.forEach(g=>{a[g]=0});const l=document.createElement("div");l.className="cheat-overlay",l.innerHTML=`
      <div class="cheat-modal">
        ...
        <div class="drinks-grid" id="cheat-drinks-grid">
          ${t.map(g=>`
            <div class="drink-row">
              <span class="drink-name">${I(g)}</span>
              <div class="drink-controls">
                <button class="drink-btn drink-minus" data-name="${I(g)}">−</button>
                <span class="drink-count" id="drink-count-${I(g.replace(/ /g,"_"))}">0</span>
                <button class="drink-btn drink-plus" data-name="${I(g)}">+</button>
              </div>
            </div>
          `).join("")}
        </div>
  ...
  `;const p=e.querySelector("#seats-bar"),a=e.querySelector("#convo-pane"),l=e.querySelector("#sidebar"),m=e.querySelector("#left-col");let u="socratic",c=0,v=null,b=!1,k=!1,g={turn:0,heat:0,concession_total:0,partial_agreements:[],remaining_disagreements:[],drift_topic:""};const y=Ee(p,t);F(l,{topic:n,turn:0,heat:0,moderator_style:"socratic",partial_agreements:[],points_of_agreement:[],remaining_disagreements:[]});{const L=document.createElement("div");L.id="debate-starting",L.className="debate-starting",L.innerHTML='<span class="debate-starting-text">Opening the bar</span><span class="typing-dots"><span>.</span><span>.</span><span>.</span></span>',a.appendChild(L)}function h(){var L;(L=a.querySelector("#debate-starting"))==null||L.remove()}function $({type:L,data:f}){switch(L){case"speaker":h(),y.setThinking(f.name),je(a,f.name);break;case"message":h(),j(a),f.backchannel||y.setSpeaking(f.name),Te(a,f);break;case"bars":c=f.heat??c,Oe(l,f.heat,f.concession_total??0);break;case"debug":{const w=f.data!=null?f.data:"",H=typeof w=="object"?`
`+Object.entries(w).map(([R,U])=>`  ${R}: ${JSON.stringify(U)}`).join(`
`):w?` — ${w}`:"";console.log(`[${f.channel}] ${f.label}${H}`);break}case"state":j(a),u=f.moderator_style,c=f.heat??c,g=f,F(l,{topic:n,...f});break;case"steer_needed":if(k)break;k=!0,u=f.current_style,f.drift_topic&&_e(a,f.drift_topic,n),a.scrollTop=a.scrollHeight,we(u,r,Ue(g),m,i.searchEvidence,t).then(w=>{k=!1,w===null?z(a,g,t,x,s,i):(u=w.style,F(l,{topic:n,...g,moderator_style:w.style}),i.steer(s,w.text,w.style,w.evidence||"",w.drinks||{}).catch(H=>D(a,`Steer error: ${H.message}`)))});break;case"consensus":if(b)break;b=!0,v&&(v(),v=null),j(a),y.clearAll(),Ae(a,f,{onNewTopic(w){i.newTopic(s,w).then(()=>{b=!1,g={turn:0,heat:0,concession_total:0,partial_agreements:[],remaining_disagreements:[],drift_topic:""},e.querySelector(".debate-topic").textContent=w,F(l,{topic:w,...g,moderator_style:u,points_of_agreement:[]}),y.clearAll(),v=i.openStream(s,$)}).catch(H=>D(a,`Error: ${H.message}`))},onQuit:x},g,s,i,t);break;case"game_over":if(b)break;b=!0,v&&(v(),v=null),j(a),y.clearAll(),z(a,f,t,x,s,i);break;case"bar_beat":h(),qe(a,f.text);break;case"commentator":h(),Ce(a,f.text);break;case"evidence":h(),He(a,f.finding);break;case"diagram":h(),Ne(a,f);break;case"system":h(),D(a,f.text);break;case"error":h(),D(a,`⚠ ${f.text}`);break}}function x(){v&&v(),i.deleteSession(s).catch(()=>{}),e.dispatchEvent(new CustomEvent("debate:quit",{bubbles:!0}))}e.querySelector("#about-btn").addEventListener("click",V),e.querySelector("#help-btn").addEventListener("click",X),e.querySelector("#sidebar-toggle").addEventListener("click",()=>{const f=e.querySelector(".debate-shell").classList.toggle("sidebar-open");e.querySelector("#sidebar-toggle").textContent=f?"Stats ▲":"Stats"}),e.querySelector("#cheat-btn").addEventListener("click",()=>{$e(s,c,t,i.cheat,()=>W(s,i,t),()=>Me(s,i))}),e.querySelector("#quit-btn").addEventListener("click",()=>{if(b){x();return}g.turn>0?(b=!0,v&&(v(),v=null),z(a,g,t,x,s,i)):x()}),v=i.openStream(s,$)}function Te(e,{role:s,name:t,content:n,backchannel:r}){const i=document.createElement("div");if(r)i.className="msg msg-bc",i.innerHTML=`<span class="bc-name">${d(t)}:</span> <em>${P(n)}</em>`;else if(s==="moderator")i.className="msg msg-moderator",i.innerHTML=`<div class="msg-mod-label">― Moderator ―</div><div class="msg-content">${P(n)}</div>`;else if(s==="user")i.className="msg msg-user",i.innerHTML=`<div class="msg-name msg-name-user">You</div><div class="msg-content">${P(n)}</div>`;else{const p=`/portraits/${t.replace(/ /g,"_")}.png`,a=t.split(" ").map(l=>l[0]).join("").slice(0,2).toUpperCase();i.className="msg msg-philosopher",i.innerHTML=`<div class="msg-avatar"><img class="msg-avatar-img" src="${p}" alt="${d(t)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="msg-avatar-initials" style="display:none">${d(a)}</div></div><div class="msg-body"><div class="msg-name">${d(t)}</div><div class="msg-content">${P(n)}</div></div>`}C(e,i)}function qe(e,s){const t=document.createElement("div");t.className="msg msg-beat",t.innerHTML=P(s),C(e,t)}function D(e,s){const t=document.createElement("div");t.className="msg msg-system",t.textContent=s,C(e,t)}function _e(e,s,t){const n=document.createElement("div");n.className="msg msg-drift",n.innerHTML=`<div class="drift-heading">⇌ Topic Drift</div><div class="drift-new">${d(s)}</div><div class="drift-orig">original: ${d(t)}</div>`,C(e,n)}function Ce(e,s){const t=document.createElement("div");t.className="msg msg-commentator",t.innerHTML=`<span class="commentator-icon">📢</span><span class="commentator-text">${d(s)}</span>`,C(e,t)}function He(e,s){const t=document.createElement("div");t.className="msg msg-evidence",t.innerHTML=`<span class="evidence-label">── EVIDENCE ──</span> ${d(s)}`,C(e,t)}function Ne(e,{speaker:s,title:t,thumb_url:n,url:r,page_url:i}){const p=document.createElement("div");p.className="msg msg-diagram",p.innerHTML=`<div class="diagram-label">${d(s)} produces a diagram</div><a class="diagram-link" href="${d(i)}" target="_blank" rel="noopener"><img class="diagram-img" src="${d(n)}" alt="${d(t)}" /><div class="diagram-caption">${d(t)}</div></a>`,C(e,p)}function Ae(e,{summary:s,points:t},{onNewTopic:n,onQuit:r},i={},p,a,l=[]){const m=document.createElement("div");m.className="consensus-panel",m.innerHTML=`
  ...
  `,C(e,p),p.querySelector("#game-over-leave").addEventListener("click",n),r&&((m=p.querySelector("#game-over-paper"))==null||m.addEventListener("click",()=>W(r,i,t)))}async function Me(e,s){const t=document.createElement("div");t.className="newspaper-overlay",t.innerHTML=`
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media print {
    ...
    <span class="report-stat-value" style="color:${a}">${l}<span style="color:var(--text-dim)">${m}</span> ${p}</span>
        ...
        </div>`:`<div class="report-tension">${d(String(c))}</div>`).join("")),u+="</div>",u}function je(e,s){j(e);const t=document.createElement("div");t.className="msg msg-typing",t.id="typing-indicator",t.innerHTML=`<span class="typing-name">${d(s)}</span><span class="typing-dots"><span>.</span><span>.</span><span>.</span></span>`,C(e,t)}function j(e){var s;(s=e.querySelector("#typing-indicator"))==null||s.remove()}function F(e,s){const{topic:t,turn:n=0,heat:r=0,concession_total:i=0,moderator_style:p="socratic",partial_agreements:a=[],points_of_agreement:l=[],remaining_disagreements:m=[]}=s;let u=`
  ...
  `}function Oe(e,s,t){const n=e.querySelector("#sb-bars");n&&(n.innerHTML=ee(s,t))}function te(e){return e<=2?"#4a7ab5":e<=4?"#8a9040":e<=6?"#c8a030":e<=8?"#c86030":"#c83030"}function se(e){return e<=2?"cool":e<=4?"warm":e<=6?"charged":e<=8?"heated":"flashpoint"}function Pe(e){return e<=2?"#4a7ab5":e<=5?"#4a9b6f":e<=8?"#6abf8a":"#a8e6bf"}function Re(e){return e===0?"none":e<=2?"rare":e<=5?"some":e<=8?"frequent":"flowing"}function Ue(e,s){const{turn:t,heat:n,partial_agreements:r,remaining_disagreements:i,drift_topic:p}=e;if(!t)return"The debate is just getting started.";if(p)return`The conversation has drifted from the original topic toward ${p}.`;const a=r||[],l=i||[];if(a.length&&l.length){const u=a[0],c=l[0],v=u.participants.join(" and "),b=typeof c=="object"?c.topic:String(c);return`${v} are finding common ground, but the group remains divided on ${b}.`}if(a.length){const u=a[0];return`${u.participants.join(" and ")} are converging on ${u.on}, ${n>=6?"though tempers are running high":"with the room following closely"}.`}if(l.length){const u=l[0];return typeof u=="object"?`${u.participant_a} and ${u.participant_b} are sharply divided over ${u.topic}.`:`The room is deadlocked — ${String(u)}.`}const m=n>=8?"at flashpoint":n>=5?"heating up":n>=3?"warming up":"still feeling each other out";return`${t} turns in, no clear alignments yet — the room is ${m}.`}const G=document.querySelector("#app");let ae={};async function ne(){let e,s;try{[e,s,ae]=await Promise.all([oe(),ce(),le()])}catch(n){G.innerHTML=`<div class="fatal-error">Could not reach the server.<br/>${n.message}</div>`;return}const t=ye(G,e,async({characters:n,topic:r,commentator:i=!0,moderator:p=!0,diagrams:a=!1,audienceLevel:l="university"})=>{try{const m=await de(n,r,i,p,a,l);Ie(m.session_id,n,r,s)}catch(m){t.showError(`Could not start session: ${m.message}`)}})}function Ie(e,s,t,n){xe(G,e,s,t,n,{steer:pe,cheat:fe,deleteSession:ve,newTopic:me,openStream:be,searchEvidence:ue,fetchNewspaper:he,exportPodcast:ae.podcast?ge:null}),G.addEventListener("debate:quit",()=>ne(),{once:!0})}ne();
