(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const l of s.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&i(l)}).observe(document,{childList:!0,subtree:!0});function t(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(a){if(a.ep)return;a.ep=!0;const s=t(a);fetch(a.href,s)}})();const E="/api";async function q(e,n){const t=await fetch(`${E}${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!t.ok){const i=await t.text();throw new Error(`${t.status} ${t.statusText}: ${i}`)}return t.json()}async function D(e){await fetch(`${E}${e}`,{method:"DELETE"})}async function G(){const e=await fetch(`${E}/characters`);if(!e.ok)throw new Error("Failed to load characters");return e.json()}async function W(){const e=await fetch(`${E}/styles`);if(!e.ok)throw new Error("Failed to load styles");return e.json()}async function F(e,n){return q("/sessions",{characters:e,topic:n})}async function Q(e,n,t){return q(`/sessions/${e}/steer`,{text:n,style:t})}async function Y(e,n){return q(`/sessions/${e}/new-topic`,{topic:n})}async function K(e){return D(`/sessions/${e}`)}function J(e,n){const t=new EventSource(`${E}/sessions/${e}/stream`);return t.onmessage=i=>{try{const a=JSON.parse(i.data);n(a)}catch{console.error("Unparseable SSE frame:",i.data)}},t.onerror=i=>{console.error("SSE error",i),n({type:"error",data:{text:"Connection lost."}})},()=>t.close()}const A="https://github.com/mhughes72/fungame03";function x(e,n){const t=document.createElement("div");t.className="info-overlay",t.innerHTML=`
  ...
  `;let a=null;function s(r){return e.querySelector(`#seat-${_(r)}`)}function l(){clearTimeout(a),e.querySelectorAll(".seat").forEach(r=>{r.classList.remove("seat-thinking","seat-speaking")})}function c(r){var o;l(),(o=s(r))==null||o.classList.add("seat-thinking")}function u(r){l();const o=s(r);o&&(o.classList.add("seat-speaking"),a=setTimeout(()=>o.classList.remove("seat-speaking"),3e3))}return{setThinking:c,setSpeaking:u,clearAll:l}}function X(e){return`/portraits/${e.replace(/ /g,"_")}.png`}function ee(e){return e.split(" ").map(n=>n[0]).join("").slice(0,2).toUpperCase()}function te(e){return e.split(" ").at(-1)}function _(e){return e.replace(/[^a-zA-Z0-9]/g,"-").toLowerCase()}function H(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function se(e,n,t,i,a,s){e.innerHTML=`
  ...
  `;const l=e.querySelector("#seats-bar"),c=e.querySelector("#convo-pane"),u=e.querySelector("#sidebar"),r=e.querySelector("#left-col");let o="socratic",d=null,g={turn:0,heat:0,partial_agreements:[],remaining_disagreements:[],drift_topic:""};const v=V(l,t);N(u,{topic:i,turn:0,heat:0,moderator_style:"socratic",partial_agreements:[],points_of_agreement:[],remaining_disagreements:[]});function h({type:U,data:m}){switch(U){case"speaker":v.setThinking(m.name),re(c,m.name);break;case"message":k(c),m.backchannel||v.setSpeaking(m.name),ne(c,m);break;case"state":o=m.moderator_style,g=m,N(u,{topic:i,...m});break;case"steer_needed":o=m.current_style,m.drift_topic&&(f(c,`── DRIFT ── conversation has shifted to: ${m.drift_topic}`),f(c,`   original topic: ${i}`)),z(o,a,oe(g),r).then(y=>{y===null?C(c,g,t,S):s.steer(n,y.text,y.style).catch(L=>f(c,`Steer error: ${L.message}`))});break;case"consensus":k(c),v.clearAll(),ie(c,m,{onNewTopic(y){s.newTopic(n,y).catch(L=>f(c,`Error: ${L.message}`))},onQuit:S},g);break;case"bar_beat":ae(c,m.text);break;case"system":f(c,m.text);break;case"error":f(c,`⚠ ${m.text}`);break}}function S(){d&&d(),s.deleteSession(n).catch(()=>{}),e.dispatchEvent(new CustomEvent("debate:quit",{bubbles:!0}))}e.querySelector("#about-btn").addEventListener("click",j),e.querySelector("#help-btn").addEventListener("click",M),e.querySelector("#quit-btn").addEventListener("click",()=>{g.turn>0?C(c,g,t,S):S()}),d=s.openStream(n,h)}function ne(e,{role:n,name:t,content:i,backchannel:a}){const s=document.createElement("div");a?(s.className="msg msg-bc",s.innerHTML=`<span class="bc-name">${p(t)}:</span> <em>${$(i)}</em>`):n==="moderator"?(s.className="msg msg-moderator",s.innerHTML=`<div class="msg-mod-label">― Moderator ―</div><div class="msg-content">${$(i)}</div>`):n==="user"?(s.className="msg msg-user",s.innerHTML=`<div class="msg-name msg-name-user">You</div><div class="msg-content">${$(i)}</div>`):(s.className="msg msg-philosopher",s.innerHTML=`<div class="msg-name">${p(t)}</div><div class="msg-content">${$(i)}</div>`),b(e,s)}function ae(e,n){const t=document.createElement("div");t.className="msg msg-beat",t.innerHTML=$(n),b(e,t)}function f(e,n){const t=document.createElement("div");t.className="msg msg-system",t.textContent=n,b(e,t)}function ie(e,{summary:n,points:t},{onNewTopic:i,onQuit:a},s={}){const l=document.createElement("div");l.className="consensus-panel",l.innerHTML=`
