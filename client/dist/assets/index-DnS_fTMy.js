(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))n(r);new MutationObserver(r=>{for(const i of r)if(i.type==="childList")for(const p of i.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&n(p)}).observe(document,{childList:!0,subtree:!0});function t(r){const i={};return r.integrity&&(i.integrity=r.integrity),r.referrerPolicy&&(i.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?i.credentials="include":r.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(r){if(r.ep)return;r.ep=!0;const i=t(r);fetch(r.href,i)}})();const _="/api";async function O(e,s){const t=await fetch(`${_}${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!t.ok){const n=await t.text();throw new Error(`${t.status} ${t.statusText}: ${n}`)}return t.json()}async function se(e){await fetch(`${_}${e}`,{method:"DELETE"})}async function G(e=0){const s=await fetch(`${_}/debate-of-the-day?index=${e}`);if(!s.ok)throw new Error("Failed to load debate of the day");return s.json()}async function ae(){const e=await fetch(`${_}/characters`);if(!e.ok)throw new Error("Failed to load characters");return e.json()}async function ne(){const e=await fetch(`${_}/styles`);if(!e.ok)throw new Error("Failed to load styles");return e.json()}async function re(){const e=await fetch(`${_}/features`);return e.ok?e.json():{}}async function ie(e,s,t=!0,n=!0,r=!1,i="university"){return O("/sessions",{characters:e,topic:s,commentator_enabled:t,moderator_enabled:n,diagrams_enabled:r,audience_level:i})}async function oe(e,s,t,n="",r={}){return O(`/sessions/${e}/steer`,{text:s,style:t,evidence:n,drinks:r})}async function ce(e){return O("/search",{query:e})}async function le(e,s){return O(`/sessions/${e}/new-topic`,{topic:s})}async function de(e){return se(`/sessions/${e}`)}async function pe(e){return O(`/sessions/${e}/newspaper`,{})}async function ue(e){const s=await fetch(`/api/sessions/${e}/podcast`,{method:"POST"});if(!s.ok){const i=await s.json().catch(()=>({detail:s.statusText}));throw new Error(i.detail||s.statusText)}const t=await s.blob(),n=URL.createObjectURL(t),r=document.createElement("a");r.href=n,r.download="philosophers-bar-podcast.mp3",document.body.appendChild(r),r.click(),r.remove(),URL.revokeObjectURL(n)}async function me(e,s,t={}){const n={drinks:t};return s!==null&&(n.heat=s),O(`/sessions/${e}/cheat`,n)}function ve(e,s){const t=new EventSource(`${_}/sessions/${e}/stream`);return t.onmessage=n=>{try{const r=JSON.parse(n.data);s(r)}catch{console.error("Unparseable SSE frame:",n.data)}},t.onerror=n=>{console.error("SSE error",n),s({type:"error",data:{text:"Connection lost."}})},()=>t.close()}const B="https://github.com/mhughes72/fungame03";function W(e,s){const t=document.createElement("div");t.className="info-overlay",t.innerHTML=`
    <div class="info-box">
  ...
  `,document.body.appendChild(t);function n(){t.remove()}t.querySelector(".info-close").addEventListener("click",n),t.addEventListener("click",r=>{r.target===t&&n()}),document.addEventListener("keydown",function r(i){i.key==="Escape"&&(n(),document.removeEventListener("keydown",r))})}function Q(){W("ABOUT",`
    ...
    <p><a class="info-link" href="${B}" target="_blank" rel="noopener">${B}</a></p>
        ...
        <div class="char-list" id="char-list">
          ${s.map(o=>`
            <label class="char-row"
              ...
              data-desc="${q(o.known_for)}"
              data-category="${q(o.category||"")}"
              data-portrait="${q(o.name.replace(/ /g,"_"))}">
  ...
  `;const n=e.querySelectorAll("#char-list input[type=checkbox]"),r=e.querySelectorAll(".char-row"),i=e.querySelector("#char-no-results"),p=e.querySelector("#char-filter");p.addEventListener("input",()=>{const o=p.value.toLowerCase().trim();let w=0;r.forEach(k=>{const x=!o||k.dataset.name.includes(o);k.style.display=x?"":"none",x&&w++}),i.style.display=w===0?"":"none"});const a=document.createElement("div");a.className="char-tooltip",a.style.display="none",document.body.appendChild(a);function l(o){const{desc:w,category:k,portrait:x}=o.currentTarget.dataset;if(!w)return;const A=`/portraits/${x}.png`;a.innerHTML=`
      <div class="tt-inner">
        <img class="tt-portrait" src="${A}" alt="" onerror="this.style.display='none'" />
        <div class="tt-body">
          ${k?`<span class="tt-category">${q(k)}</span>`:""}
          <span class="tt-desc">${q(w)}</span>
        </div>
      </div>`,a.style.display="block",m(o)}function m(o){const k=a.offsetWidth,x=a.offsetHeight;let A=o.clientX+14,U=o.clientY+14;A+k>window.innerWidth-14&&(A=o.clientX-k-14),U+x>window.innerHeight-14&&(U=o.clientY-x-14),a.style.left=A+"px",a.style.top=U+"px"}function u(){a.style.display="none"}r.forEach(o=>{o.addEventListener("mouseenter",l),o.addEventListener("mousemove",m),o.addEventListener("mouseleave",u)});const c=new MutationObserver(()=>{document.body.contains(e)||(a.remove(),c.disconnect())});c.observe(document.body,{childList:!0,subtree:!0});const v=e.querySelector("#selection-hint"),f=e.querySelector("#start-btn"),E=e.querySelector("#setup-error");function g(){const o=[...n].filter(w=>w.checked).length;o<2?(v.textContent=o===0?"Select 2 to 4 thinkers":"Select 1 more",v.classList.remove("hint-ok","hint-warn")):o>4?(v.textContent=`Too many — deselect ${o-4}`,v.classList.add("hint-warn"),v.classList.remove("hint-ok")):(v.textContent=`${o} selected`,v.classList.add("hint-ok"),v.classList.remove("hint-warn")),f.disabled=o<2||o>4}g(),n.forEach(o=>o.addEventListener("change",g));function y(){const o=e.querySelector('input[name="audience"]:checked');return{commentator:e.querySelector("#toggle-commentator").checked,moderator:e.querySelector("#toggle-moderator").checked,diagrams:e.querySelector("#toggle-diagrams").checked,audienceLevel:o?o.value:"university"}}f.addEventListener("click",()=>{const o=[...n].filter(k=>k.checked).map(k=>k.value),w=e.querySelector("#topic-input").value.trim()||"What is the nature of justice?";E.textContent="",t({characters:o,topic:w,...y()})}),e.querySelector("#topic-input").addEventListener("keydown",o=>{o.key==="Enter"&&!f.disabled&&f.click()}),e.querySelector("#setup-about").addEventListener("click",Q),e.querySelector("#setup-help").addEventListener("click",K);const h=e.querySelector("#dotd-card"),$={heated:"var(--red)",historic:"var(--blue)",philosophical:"var(--gold-dim)",scientific:"var(--blue)",cultural:"var(--amber)",political:"var(--green)"};let S=0;function L(o){const w=$[o.category]||"var(--text-dim)";h.innerHTML=`
      ...
      <div class="dotd-topic">${q(o.topic)}</div>
      <div class="dotd-tagline">${q(o.tagline)}</div>
    ...
    `,h.querySelector("#dotd-start").addEventListener("click",()=>{t({characters:o.characters,topic:o.topic,...y()})}),h.querySelector("#dotd-new").addEventListener("click",()=>{S++,b(S)})}function b(o){h.innerHTML='<div class="dotd-loading">generating…</div>',G(o).then(L).catch(()=>{o===0?h.style.display="none":(S--,G(S).then(L).catch(()=>{h.style.display="none"}))})}return b(0),{showError(o){E.textContent=o}}}function q(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function C(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function ge(e,s,t="",n,r=null,i=[]){return new Promise(p=>{const a=document.createElement("div");a.className="steer-drawer",a.innerHTML=`
      ...
      ${t?`<div class="steer-summary">${C(t)}</div>`:""}
      ...
      <div class="style-list" id="style-list">
        ${s.map(h=>`
          <button
            ...
            data-style="${C(h.style)}"
          >
            <span class="style-name">${C(h.style)}</span>
            <span class="style-desc">${C(h.description)}</span>
          </button>
        `).join("")}
      </div>
    `,(n||document.body).appendChild(a);const m=a.querySelector("#steer-text-input"),u=a.querySelector("#evidence-query"),c=a.querySelector("#evidence-search"),v=a.querySelector("#evidence-preview");m.focus();let f=e,E="";async function g(){const h=u.value.trim();if(!(!h||!r)){c.disabled=!0,c.textContent="Searching…",v.style.display="none",E="";try{const $=await r(h);E=$.finding,v.style.display="block",v.innerHTML=`
          <div class="evidence-preview-label">── FINDING ──</div>
          <div class="evidence-preview-text">${C($.finding)}</div>
        ...
        `,v.querySelector("#evidence-accept").addEventListener("click",()=>{v.innerHTML=`<div class="evidence-accepted">✓ Evidence will be injected: ${C(E)}</div>`}),v.querySelector("#evidence-discard").addEventListener("click",()=>{E="",v.style.display="none"})}catch($){v.style.display="block",v.textContent=`Search failed: ${$.message}`}finally{c.disabled=!1,c.textContent="Search"}}}c.addEventListener("click",g),u.addEventListener("keydown",h=>{h.key==="Enter"&&g()}),a.querySelectorAll(".style-item").forEach(h=>{h.addEventListener("click",()=>{a.querySelectorAll(".style-item").forEach($=>$.classList.remove("style-selected")),h.classList.add("style-selected"),f=h.dataset.style,y()})});function y(){const h=m.value.trim();a.remove(),p({text:h,style:f,evidence:E})}a.querySelector("#steer-submit").addEventListener("click",y),a.querySelector("#steer-quit").addEventListener("click",()=>{a.remove(),p(null)}),m.addEventListener("keydown",h=>{h.key==="Enter"&&y()})})}function M(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const z=["arctic","cool","cool","warm","warm","charged","charged","heated","heated","flashpoint","flashpoint"];function be(e,s,t,n,r=null,i=null){return new Promise(p=>{var f,E;const a={};t.forEach(g=>{a[g]=0});const l=document.createElement("div");l.className="cheat-overlay",l.innerHTML=`
      <div class="cheat-modal">
        ...
        <div class="drinks-grid" id="cheat-drinks-grid">
          ${t.map(g=>`
            <div class="drink-row">
              <span class="drink-name">${M(g)}</span>
              <div class="drink-controls">
                <button class="drink-btn drink-minus" data-name="${M(g)}">−</button>
                <span class="drink-count" id="drink-count-${M(g.replace(/ /g,"_"))}">0</span>
                <button class="drink-btn drink-plus" data-name="${M(g)}">+</button>
              </div>
            </div>
          `).join("")}
        </div>
    ...
    `,document.body.appendChild(l);const m=l.querySelector("#cheat-heat-slider"),u=l.querySelector("#cheat-heat-value");m.addEventListener("input",()=>{const g=parseInt(m.value,10);u.textContent=`${g} — ${z[g]}`}),l.querySelectorAll(".drink-btn").forEach(g=>{g.addEventListener("click",()=>{const y=g.dataset.name,h=g.classList.contains("drink-plus")?1:-1;a[y]=Math.max(0,(a[y]||0)+h);const $=y.replace(/ /g,"_"),S=l.querySelector(`#drink-count-${$}`);S&&(S.textContent=a[y])})});function c(){l.remove(),p()}async function v(){const g=parseInt(m.value,10),y=Object.fromEntries(Object.entries(a).filter(([,$])=>$>0)),h=g!==s;try{await n(e,h?g:null,y)}catch($){console.error("Cheat failed:",$)}c()}l.querySelector("#cheat-apply").addEventListener("click",v),l.querySelector("#cheat-close").addEventListener("click",c),(f=l.querySelector("#cheat-paper"))==null||f.addEventListener("click",()=>{c(),r()}),(E=l.querySelector("#cheat-podcast"))==null||E.addEventListener("click",()=>{c(),i()}),l.addEventListener("click",g=>{g.target===l&&c()})})}function fe(e,s){e.innerHTML=s.map(a=>{const l=ye(a),m=we(a);return`
      <div class="seat" id="seat-${Y(a)}">
        <div class="seat-portrait-wrap">
          <img class="seat-img" src="${l}" alt="${I(a)}"
               onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" />
          <div class="seat-initials" style="display:none">${I(m)}</div>
        </div>
        <div class="seat-name">${I($e(a))}</div>
      </div>
    `}).join("");let t=null;function n(a){return e.querySelector(`#seat-${Y(a)}`)}function r(){clearTimeout(t),e.querySelectorAll(".seat").forEach(a=>{a.classList.remove("seat-thinking","seat-speaking")})}function i(a){var l;r(),(l=n(a))==null||l.classList.add("seat-thinking")}function p(a){r();const l=n(a);l&&(l.classList.add("seat-speaking"),t=setTimeout(()=>l.classList.remove("seat-speaking"),3e3))}return{setThinking:i,setSpeaking:p,clearAll:r}}function ye(e){return`/portraits/${e.replace(/ /g,"_")}.png`}function we(e){return e.split(" ").map(s=>s[0]).join("").slice(0,2).toUpperCase()}function $e(e){return e.split(" ").at(-1)}function Y(e){return e.replace(/[^a-zA-Z0-9]/g,"-").toLowerCase()}function I(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Ee(e,s,t,n,r,i){e.innerHTML=`
  ...
  `;const p=e.querySelector("#seats-bar"),a=e.querySelector("#convo-pane"),l=e.querySelector("#sidebar"),m=e.querySelector("#left-col");let u="socratic",c=0,v=null,f=!1,E=!1,g={turn:0,heat:0,concession_total:0,partial_agreements:[],remaining_disagreements:[],drift_topic:""};const y=fe(p,t);P(l,{topic:n,turn:0,heat:0,moderator_style:"socratic",partial_agreements:[],points_of_agreement:[],remaining_disagreements:[]});{const L=document.createElement("div");L.id="debate-starting",L.className="debate-starting",L.innerHTML='<span class="debate-starting-text">Opening the bar</span><span class="typing-dots"><span>.</span><span>.</span><span>.</span></span>',a.appendChild(L)}function h(){var L;(L=a.querySelector("#debate-starting"))==null||L.remove()}function $({type:L,data:b}){switch(L){case"speaker":h(),y.setThinking(b.name),Ce(a,b.name);break;case"message":h(),N(a),b.backchannel||y.setSpeaking(b.name),Se(a,b);break;case"bars":c=b.heat??c,He(l,b.heat,b.concession_total??0);break;case"debug":{const o=b.data!=null?b.data:"",w=typeof o=="object"?`
`+Object.entries(o).map(([k,x])=>`  ${k}: ${JSON.stringify(x)}`).join(`
`):o?` — ${o}`:"";console.log(`[${b.channel}] ${b.label}${w}`);break}case"state":N(a),u=b.moderator_style,c=b.heat??c,g=b,P(l,{topic:n,...b});break;case"steer_needed":if(E)break;E=!0,u=b.current_style,b.drift_topic&&(H(a,`── DRIFT ── conversation has shifted to: ${b.drift_topic}`),H(a,`   original topic: ${n}`)),a.scrollTop=a.scrollHeight,ge(u,r,Ae(g),m,i.searchEvidence,t).then(o=>{E=!1,o===null?D(a,g,t,S,s,i):(u=o.style,P(l,{topic:n,...g,moderator_style:o.style}),i.steer(s,o.text,o.style,o.evidence||"",o.drinks||{}).catch(w=>H(a,`Steer error: ${w.message}`)))});break;case"consensus":if(f)break;f=!0,v&&(v(),v=null),N(a),y.clearAll(),qe(a,b,{onNewTopic(o){i.newTopic(s,o).then(()=>{f=!1,g={turn:0,heat:0,concession_total:0,partial_agreements:[],remaining_disagreements:[],drift_topic:""},e.querySelector(".debate-topic").textContent=o,P(l,{topic:o,...g,moderator_style:u,points_of_agreement:[]}),y.clearAll(),v=i.openStream(s,$)}).catch(w=>H(a,`Error: ${w.message}`))},onQuit:S},g,s,i,t);break;case"game_over":if(f)break;f=!0,v&&(v(),v=null),N(a),y.clearAll(),D(a,b,t,S,s,i);break;case"bar_beat":h(),ke(a,b.text);break;case"commentator":h(),Le(a,b.text);break;case"evidence":h(),xe(a,b.finding);break;case"diagram":h(),Te(a,b);break;case"system":h(),H(a,b.text);break;case"error":h(),H(a,`⚠ ${b.text}`);break}}function S(){v&&v(),i.deleteSession(s).catch(()=>{}),e.dispatchEvent(new CustomEvent("debate:quit",{bubbles:!0}))}e.querySelector("#about-btn").addEventListener("click",Q),e.querySelector("#help-btn").addEventListener("click",K),e.querySelector("#sidebar-toggle").addEventListener("click",()=>{const b=e.querySelector(".debate-shell").classList.toggle("sidebar-open");e.querySelector("#sidebar-toggle").textContent=b?"Stats ▲":"Stats"}),e.querySelector("#cheat-btn").addEventListener("click",()=>{be(s,c,t,i.cheat,()=>F(s,i,t),()=>_e(s,i))}),e.querySelector("#quit-btn").addEventListener("click",()=>{if(f){S();return}g.turn>0?(f=!0,v&&(v(),v=null),D(a,g,t,S,s,i)):S()}),v=i.openStream(s,$)}function Se(e,{role:s,name:t,content:n,backchannel:r}){const i=document.createElement("div");if(r)i.className="msg msg-bc",i.innerHTML=`<span class="bc-name">${d(t)}:</span> <em>${j(n)}</em>`;else if(s==="moderator")i.className="msg msg-moderator",i.innerHTML=`<div class="msg-mod-label">― Moderator ―</div><div class="msg-content">${j(n)}</div>`;else if(s==="user")i.className="msg msg-user",i.innerHTML=`<div class="msg-name msg-name-user">You</div><div class="msg-content">${j(n)}</div>`;else{const p=`/portraits/${t.replace(/ /g,"_")}.png`,a=t.split(" ").map(l=>l[0]).join("").slice(0,2).toUpperCase();i.className="msg msg-philosopher",i.innerHTML=`<div class="msg-avatar"><img class="msg-avatar-img" src="${p}" alt="${d(t)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="msg-avatar-initials" style="display:none">${d(a)}</div></div><div class="msg-body"><div class="msg-name">${d(t)}</div><div class="msg-content">${j(n)}</div></div>`}T(e,i)}function ke(e,s){const t=document.createElement("div");t.className="msg msg-beat",t.innerHTML=j(s),T(e,t)}function H(e,s){const t=document.createElement("div");t.className="msg msg-system",t.textContent=s,T(e,t)}function Le(e,s){const t=document.createElement("div");t.className="msg msg-commentator",t.innerHTML=`<span class="commentator-icon">📢</span><span class="commentator-text">${d(s)}</span>`,T(e,t)}function xe(e,s){const t=document.createElement("div");t.className="msg msg-evidence",t.innerHTML=`<span class="evidence-label">── EVIDENCE ──</span> ${d(s)}`,T(e,t)}function Te(e,{speaker:s,title:t,thumb_url:n,url:r,page_url:i}){const p=document.createElement("div");p.className="msg msg-diagram",p.innerHTML=`<div class="diagram-label">${d(s)} produces a diagram</div><a class="diagram-link" href="${d(i)}" target="_blank" rel="noopener"><img class="diagram-img" src="${d(n)}" alt="${d(t)}" /><div class="diagram-caption">${d(t)}</div></a>`,T(e,p)}function qe(e,{summary:s,points:t},{onNewTopic:n,onQuit:r},i={},p,a,l=[]){const m=document.createElement("div");m.className="consensus-panel",m.innerHTML=`
  ...
  `,T(e,p),p.querySelector("#game-over-leave").addEventListener("click",n),r&&((m=p.querySelector("#game-over-paper"))==null||m.addEventListener("click",()=>F(r,i,t)))}async function _e(e,s){const t=document.createElement("div");t.className="newspaper-overlay",t.innerHTML=`
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media print {
    ...
    <span class="report-stat-value" style="color:${a}">${l}<span style="color:var(--text-dim)">${m}</span> ${p}</span>
        ...
        </div>`:`<div class="report-tension">${d(String(c))}</div>`).join("")),u+="</div>",u}function Ce(e,s){N(e);const t=document.createElement("div");t.className="msg msg-typing",t.id="typing-indicator",t.innerHTML=`<span class="typing-name">${d(s)}</span><span class="typing-dots"><span>.</span><span>.</span><span>.</span></span>`,T(e,t)}function N(e){var s;(s=e.querySelector("#typing-indicator"))==null||s.remove()}function P(e,s){const{topic:t,turn:n=0,heat:r=0,concession_total:i=0,moderator_style:p="socratic",partial_agreements:a=[],points_of_agreement:l=[],remaining_disagreements:m=[]}=s;let u=`
  ...
  `,e.innerHTML=u}function j(e){return d(e).replace(/\*\[([^\]]+)\]\*/g,'<em class="stage-dir">[$1]</em>').replace(/\*([^*\n]+)\*/g,'<em class="stage-dir">[$1]</em>').replace(/\n/g,"<br>")}function T(e,s){const t=e.scrollHeight-e.scrollTop-e.clientHeight<120;e.appendChild(s),t&&(e.scrollTop=e.scrollHeight)}function d(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function V(e,s){const t=X(e),n=Z(e),r="█".repeat(e),i="░".repeat(10-e),p=Math.min(s,10),a=Ne(s),l="█".repeat(p),m="░".repeat(10-p),u=Oe(s);return`
  ...
  `}function He(e,s,t){const n=e.querySelector("#sb-bars");n&&(n.innerHTML=V(s,t))}function X(e){return e<=2?"#4a7ab5":e<=4?"#8a9040":e<=6?"#c8a030":e<=8?"#c86030":"#c83030"}function Z(e){return e<=2?"cool":e<=4?"warm":e<=6?"charged":e<=8?"heated":"flashpoint"}function Ne(e){return e<=2?"#4a7ab5":e<=5?"#4a9b6f":e<=8?"#6abf8a":"#a8e6bf"}function Oe(e){return e===0?"none":e<=2?"rare":e<=5?"some":e<=8?"frequent":"flowing"}function Ae(e,s){const{turn:t,heat:n,partial_agreements:r,remaining_disagreements:i,drift_topic:p}=e;if(!t)return"The debate is just getting started.";if(p)return`The conversation has drifted from the original topic toward ${p}.`;const a=r||[],l=i||[];if(a.length&&l.length){const u=a[0],c=l[0],v=u.participants.join(" and "),f=typeof c=="object"?c.topic:String(c);return`${v} are finding common ground, but the group remains divided on ${f}.`}if(a.length){const u=a[0];return`${u.participants.join(" and ")} are converging on ${u.on}, ${n>=6?"though tempers are running high":"with the room following closely"}.`}if(l.length){const u=l[0];return typeof u=="object"?`${u.participant_a} and ${u.participant_b} are sharply divided over ${u.topic}.`:`The room is deadlocked — ${String(u)}.`}const m=n>=8?"at flashpoint":n>=5?"heating up":n>=3?"warming up":"still feeling each other out";return`${t} turns in, no clear alignments yet — the room is ${m}.`}const R=document.querySelector("#app");let ee={};async function te(){let e,s;try{[e,s,ee]=await Promise.all([ae(),ne(),re()])}catch(n){R.innerHTML=`<div class="fatal-error">Could not reach the server.<br/>${n.message}</div>`;return}const t=he(R,e,async({characters:n,topic:r,commentator:i=!0,moderator:p=!0,diagrams:a=!1,audienceLevel:l="university"})=>{try{const m=await ie(n,r,i,p,a,l);je(m.session_id,n,r,s)}catch(m){t.showError(`Could not start session: ${m.message}`)}})}function je(e,s,t,n){Ee(R,e,s,t,n,{steer:oe,cheat:me,deleteSession:de,newTopic:le,openStream:ve,searchEvidence:ce,fetchNewspaper:pe,exportPodcast:ee.podcast?ue:null}),R.addEventListener("debate:quit",()=>te(),{once:!0})}te();
