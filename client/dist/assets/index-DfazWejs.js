...
`):g?` — ${g}`:"";console.log(`[${h.channel}] ${h.label}${T}`);break}case"state":l=h.moderator_style,c=h.heat??c,b=h,N(o,{topic:n,...h});break;case"steer_needed":l=h.current_style,h.drift_topic&&(k(a,`── DRIFT ── conversation has shifted to: ${h.drift_topic}`),k(a,`   original topic: ${n}`)),a.scrollTop=a.scrollHeight,ce(l,i,we(b),u,r.searchEvidence,t).then(g=>{g===null?A(a,b,t,m,s,r):(l=g.style,N(o,{topic:n,...b,moderator_style:g.style}),r.steer(s,g.text,g.style,g.evidence||"",g.drinks||{}).catch(T=>k(a,`Steer error: ${T.message}`)))});break;case"consensus":if(f)break;f=!0,v&&(v(),v=null),_(a),y.clearAll(),fe(a,h,{onNewTopic(g){r.newTopic(s,g).then(()=>{f=!1,b={turn:0,heat:0,concession_total:0,partial_agreements:[],remaining_disagreements:[],drift_topic:""},e.querySelector(".debate-topic").textContent=g,N(o,{topic:g,...b,moderator_style:l,points_of_agreement:[]}),y.clearAll(),v=r.openStream(s,w)}).catch(T=>k(a,`Error: ${T.message}`))},onQuit:m},b,s,r);break;case"game_over":if(f)break;f=!0,v&&(v(),v=null),_(a),y.clearAll(),A(a,h,t,m,s,r);break;case"bar_beat":me(a,h.text);break;case"evidence":be(a,h.finding);break;case"system":k(a,h.text);break;case"error":k(a,`⚠ ${h.text}`);break}}function m(){v&&v(),r.deleteSession(s).catch(()=>{}),e.dispatchEvent(new CustomEvent("debate:quit",{bubbles:!0}))}e.querySelector("#about-btn").addEventListener("click",B),e.querySelector("#help-btn").addEventListener("click",U),e.querySelector("#cheat-btn").addEventListener("click",()=>{oe(s,c,t,r.cheat)}),e.querySelector("#quit-btn").addEventListener("click",()=>{if(f){m();return}b.turn>0?(f=!0,v&&(v(),v=null),A(a,b,t,m,s,r)):m()}),v=r.openStream(s,w)}function he(e,{role:s,name:t,content:n,backchannel:i}){const r=document.createElement("div");if(i)r.className="msg msg-bc",r.innerHTML=`<span class="bc-name">${p(t)}:</span> <em>${q(n)}</em>`;else if(s==="moderator")r.className="msg msg-moderator",r.innerHTML=`<div class="msg-mod-label">― Moderator ―</div><div class="msg-content">${q(n)}</div>`;else if(s==="user")r.className="msg msg-user",r.innerHTML=`<div class="msg-name msg-name-user">You</div><div class="msg-content">${q(n)}</div>`;else{const d=`/portraits/${t.replace(/ /g,"_")}.png`,a=t.split(" ").map(o=>o[0]).join("").slice(0,2).toUpperCase();r.className="msg msg-philosopher",r.innerHTML=`<div class="msg-avatar"><img class="msg-avatar-img" src="${d}" alt="${p(t)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="msg-avatar-initials" style="display:none">${p(a)}</div></div><div class="msg-body"><div class="msg-name">${p(t)}</div><div class="msg-content">${q(n)}</div></div>`}E(e,r)}function me(e,s){const t=document.createElement("div");t.className="msg msg-beat",t.innerHTML=q(s),E(e,t)}function k(e,s){const t=document.createElement("div");t.className="msg msg-system",t.textContent=s,E(e,t)}function be(e,s){const t=document.createElement("div");t.className="msg msg-evidence",t.innerHTML=`<span class="evidence-label">── EVIDENCE ──</span> ${p(s)}`,E(e,t)}function fe(e,{summary:s,points:t},{onNewTopic:n,onQuit:i},r={},d,a){const o=document.createElement("div");o.className="consensus-panel",o.innerHTML=`
